...
- Changing this contract requires a plan item, not a drive-by migration.
...
- Feedback runtime: `apps/acme-admin/src/routes/(app)/+layout.svelte` creates
  ...
  feedback contract could absorb it, but until a plan item says otherwise
  the structural-shell rule is what downstream apps follow.
...
- Why it remains: these stacks orchestrate paged loading, entity CRUD
  ...
  primitive migration and stays out of scope until a plan item says
  otherwise.
...
plan item moves it. See Q-001.

- Owner today: the containing group's owner (Underlay, or `acme-admin` for
  the group 3 entry). Any Poodle ownership transfer requires a plan
  decision.
...
and must not drift into a migration wave without a plan item.
