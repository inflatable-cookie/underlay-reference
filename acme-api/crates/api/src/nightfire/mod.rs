  ...
  R: NightfireBlockMediaHandlerRegistry,
{
  if let Some(value) = value {
    ...
    extractor
      ...
      .await
      .map_err(|err| {
        ApiError::internal(sync_error_code, sync_error_message).with_context(serde_json::json!({
    ...
    sync_media_usages_for_record(repo, used_by_type, used_by_id, &[], &provenance_kind)
      .await
      .map_err(|err| {
        ApiError::internal(clear_error_code, clear_error_message).with_context(serde_json::json!({
